...
const IngresoTipificacion = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      setEspecificacionOptions([
        ...
        { value: "Defensa no le informa de su causa", label: "Defensa no le informa de su causa" },
        { value: "Defensa no realiza diligencias oportunamente", label: "Defensa no realiza diligencias oportunamente" },
        { value: "Defensor no agiliza la libertad del imputado", label: "Defensor no agiliza la libertad del imputado" },
        { value: "Defensor no hace nada por el avance de la causa", label: "Defensor no hace nada por el avance de la causa" },
        { value: "Defensor no presenta recurso, siendo necesario", label: "Defensor no presenta recurso, siendo necesario" },
        { value: "Defensor no se entrevista con imputado (a)", label: "Defensor no se entrevista con imputado (a)" },
        { value: "Defensor no solicita peritajes, siendo necesario", label: "Defensor no solicita peritajes, siendo necesario" },
        { value: "Defensor no visita al o a la imputado(a) en prisión", label: "Defensor no visita al o a la imputado(a) en prisión" },
        { value: "Mal trato del o de la defensor", label: "Mal trato del o de la defensor" },
        ...
        { value: "Postergación de citas de entrevista en oficina", label: "Postergación de citas de entrevista en oficina" },
        { value: "Se solicita cambio de Defensor", label: "Se solicita cambio de Defensor" }
      ]);
    };
  ...
  return (
    <Box sx={{ padding: 2, display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <TextField id="justificacion" label="Justificación de la Especificación"
        value={justificacion}
      ...
      >
        <Button
          variant="outlined"
          sx={{ maxWidth: '150px', fontSize: '0.875rem', padding: '6px 12px', borderColor: '#001f3f', color: '#001f3f' }}
          onClick={handleBack}
          startIcon={<ArrowLeft />}
        >
          Volver
        ...
        <Button
          variant="contained"
          sx={{ backgroundColor: '#001f3f', color: '#fff', maxWidth: '150px', fontSize: '0.875rem', padding: '6px 12px' }}
          onClick={handleSave} disabled={!tipoSolicitud || !especificacion || !justificacion}
          startIcon={<Save />}
        >
          Grabar
        </Button>
