...
import { TextField, Select, MenuItem, FormControl, InputLabel, Box, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TablePagination, IconButton, Button } from '@mui/material';
...
const initialData = [
  ...
  { id: 52798, region: 'Antofagasta', usuario: 'RAFAEL EDUARDO TORRES', fechaApelacion: '10-11-2024', fechaTope: '29-11-2024', quienApela: 'Beneficiario', requerimiento: 'Defensa no presenta pruebas en tiempo' },
];
          ...
          <TableBody>
            {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((dato) => (
              <TableRow key={dato.id} sx={{ transition: 'background-color 0.3s', cursor: 'pointer', '&:hover': { backgroundColor: '#bbdefb' } }}>
                ...
                <TableCell sx={{ color: (() => {
                                    const [day, month, year] = dato.fechaTope.split('-');
                                    const fechaTope = new Date(`${year}-${month}-${day}`);
                                    return fechaTope < new Date() ? 'red' : 'inherit';
                                  })() }}>{dato.fechaTope}
                                  </TableCell>
                <TableCell>{dato.quienApela}</TableCell>
                <TableCell>
                  <IconButton color="primary" component={Link} to={'../secciones'}>
                    <VisibilityIcon />
                  </IconButton>
                </TableCell>
              </TableRow>
