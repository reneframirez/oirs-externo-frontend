...
import { Button, Card, CardContent, CardHeader, TextField, Radio, RadioGroup, FormControl, FormControlLabel, FormLabel, MenuItem, Select, InputLabel, Grid, Typography } from '@mui/material';
import { User, Mail, Phone, MapPin, Flag, Book, Users, FileText, Building, Gavel, Calendar, Globe } from 'lucide-react';
...
export default function IngresoSolicitudInterno() {
  ...
  const comunas = [
    'Santiago', 'Concepción', 'Valparaíso', 'La Serena', 'Antofagasta',
  ...
  const escolaridades = [
    ...
    'Educación Media Incompleta', 'Educación Media Completa',
    'Educación Superior Técnica Incompleta', 'Educación Superior Técnica Completa',
    'Educación Universitaria Incompleta', 'Educación Universitaria Completa',
    'Postgrado'
  ];

  const etnias = [
    'Mapuche', 'Aymara', 'Rapa Nui', 'Lican Antai', 'Quechua',
    'Colla', 'Diaguita', 'Kawésqar', 'Yagán', 'No pertenece a ningún pueblo indígena'
  ];

  const tiposRecepcion = [
    'Presencial', 'Correo Electrónico', 'Teléfono', 'Carta', 'Formulario Web'
  ];

  const institucionesPublicas = [
    'Ministerio de Educación', 'Ministerio de Salud', 'Ministerio del Interior',
    'Servicio de Impuestos Internos', 'Registro Civil', 'Ministerio de Vivienda y Urbanismo',
    'Ministerio de Desarrollo Social', 'Ministerio del Trabajo', 'Ministerio de Transportes y Telecomunicaciones'
  ];
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <Typography variant="h5">Información del formulario</Typography>
      </CardHeader>
      <CardContent>
        <form className="space-y-8">
          ...
          <Typography variant="h6" gutterBottom>Antecedentes del usuario</Typography>
              ...
              <FormControl fullWidth>
                ...
                >
                  {escolaridades.map((escolaridad) => (
                    <MenuItem key={escolaridad} value={escolaridad}>{escolaridad}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Grid>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                >
                  {etnias.map((etnia) => (
                    <MenuItem key={etnia} value={etnia}>{etnia}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Grid>
       ...
       <Typography variant="h6" gutterBottom>Antecedentes del Requerimiento</Typography>
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                <InputLabel id="tipo-recepcion-label">Tipo de Recepción Requerimiento</InputLabel>
                <Select
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <FormControl fullWidth>
                <InputLabel id="institucion-publica-label">Institución Pública</InputLabel>
                <Select
                ...
                >
                  {institucionesPublicas.map((institucion) => (
                    <MenuItem key={institucion} value={institucion}>{institucion}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Grid>
          ...
          <div className="flex justify-end space-x-4">
            ...
            <Button variant="contained" type="submit">Grabar</Button>
          </div>
        </form>
      </CardContent>
    </Card>
